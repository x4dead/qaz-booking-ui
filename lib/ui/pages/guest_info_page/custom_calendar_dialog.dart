...
///    [CustomCalendarDialog] and provides an optional compact view where the
...
class CustomCalendarDialog extends StatefulWidget {
  ...
  /// change the [key] to create a new instance of the [CustomCalendarDialog], and
...
class _CustomCalendarDialogState extends State<CustomCalendarDialog> {
  ...
  late final ValueNotifier<DatePickerMode> _mode =
      ValueNotifier(widget.initialCalendarMode);
  late final ValueNotifier<DateTime> _currentDisplayedMonthDate = ValueNotifier(
      DateTime(widget.initialDate?.year ?? widget.currentDate.year,
          widget.initialDate?.month ?? widget.currentDate.month));
  late final ValueNotifier<DateTime?> _selectedDate =
      ValueNotifier(widget.initialDate ?? widget.initialDate!);
  ...
  void didChangeDependencies() {
    ...
    if (!_announcedInitialDate && widget.initialDate != null) {
      assert(_selectedDate.value != null);
      _announcedInitialDate = true;
      final bool isToday =
          DateUtils.isSameDay(widget.currentDate, _selectedDate.value);
      final String semanticLabelSuffix =
          isToday ? ', ${_localizations.currentDateLabel}' : '';
      SemanticsService.announce(
        '${_localizations.formatFullDate(_selectedDate.value!)}$semanticLabelSuffix',
        _textDirection,
      );
    }
  }
  ...
  void _handleModeChanged(DatePickerMode mode) {
    ...
    if (_selectedDate.value != null) {
      if (_mode.value == DatePickerMode.day) {
        SemanticsService.announce(
          _localizations.formatMonthYear(_selectedDate.value!),
          _textDirection,
        );
      } else {
        SemanticsService.announce(
          _localizations.formatYear(_selectedDate.value!),
          _textDirection,
        );
      }
    }
  ...
  void _handleMonthChanged(DateTime date) {
    if (_currentDisplayedMonthDate.value.year != date.year ||
        _currentDisplayedMonthDate.value.month != date.month) {
      _currentDisplayedMonthDate.value = DateTime(date.year, date.month);
      widget.onDisplayedMonthChanged?.call(_currentDisplayedMonthDate.value);
    }
  }
  ...
  void _handleYearChanged(DateTime value) {
    ...
    final int preferredDay =
        math.min(_selectedDate.value?.day ?? 1, daysInMonth);
    ...
    _mode.value = DatePickerMode.day;
  ...
  Widget _buildPicker() {
    return ValueListenableBuilder(
        ...
        builder: (context, value, child) => switch (_mode.value) {
              DatePickerMode.day => _MonthPicker(
                  key: _monthPickerKey,
                  initialMonth: _currentDisplayedMonthDate.value,
              ...
              DatePickerMode.year => Padding(
                  padding: const EdgeInsets.only(top: 39 + 8),
                  child: YearPicker(
                    key: _yearPickerKey,
                    currentDate: widget.currentDate,
                    firstDate: widget.firstDate,
                    lastDate: widget.lastDate,
                    selectedDate: _currentDisplayedMonthDate.value,
  ...
  String formatMonthYear(DateTime date) {
    ...
    final String month = months[date.month - DateTime.january];
    return '$month $year';
  }
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
              ...
              child: Stack(children: [
                ValueListenableBuilder(
                    ...
                    builder: (context, v, child) {
                      return Center(
                        child: Text(
                          "${weekDays[_selectedDate.value!.weekday - 1]}, ${_selectedDate.value?.day}",
          ...
          Padding(
            padding: kPAll20.copyWith(top: 6),
            child: Column(
              children: [
                Stack(
                  children: [
                    ...
                    ValueListenableBuilder(
                        valueListenable: _currentDisplayedMonthDate,
                        builder: (context, v, child) {
                          return Align(
                ...
                Row(
                  children: [
                    Expanded(
                        child: CustomButton(
                      buttonText: 'Отмена',
                      textColor: AppColors.colorOrange,
                      bgColor: AppColors.colorTertiaryOrange,
                      highlightColor:
                          AppColors.colorSecondaryOrange.withOpacity(0.5),
                      onTap: () {
                        context.pop();
                      },
                    )),
                    kSBW15,
                    CustomButton(
                      bgColor: AppColors.colorTertiaryBlue,
                      buttonText: 'Готово',
                      textColor: AppColors.colorBlue,
                      highlightColor:
                          AppColors.colorSecondaryBlue.withOpacity(0.5),
                      width: 171,
                      onTap: () {
                        widget.onDateChanged(_selectedDate.value!);
                        context.pop();
...
class _MonthPicker extends StatefulWidget {
  ...
  _MonthPicker({
    ...
    this.selectableDayPredicate,
        ...
        assert(selectedDate.value == null ||
            !selectedDate.value!.isBefore(firstDate)),
        assert(selectedDate.value == null ||
            !selectedDate.value!.isAfter(lastDate));
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void _showMonth(DateTime month, {bool jump = false}) {
    ...
    if (jump) {
      pageController.jumpToPage(monthPage);
    } else {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (DateUtils.isSameMonth(widget.selectedDate.value, _currentMonth)) {
  ...
  Widget handleButton({bool isRightButton = false}) {
    return SizedBox(
      height: 39,
      width: 39,
      child: IconButton(
        icon: Icon(isRightButton == true
            ? Icons.chevron_right_rounded
            : Icons.chevron_left_rounded),
        color: AppColors.colorBlack,
        tooltip: isRightButton == false
        ...
        children: [
          SizedBox(
            ...
            child: Row(
              ...
              children: [
                handleButton(),
                handleButton(isRightButton: true),
              ],
            ),
          ),
          kSBH8,
...
class _DayPickerState extends State<_DayPicker> {
  ...
  List<Widget> _dayHeaders() {
    return List.generate(math.max(0, 7 * 2 - 1), (index) {
      if (index.isEven) {
        ...
        return ExcludeSemantics(
          child: SizedBox(
            ...
            width: 40,
            child: Center(
                child: Text(shortWeekDays[itemIndex],
                    style: AppTextStyle.w400s14.copyWith(
                        height: 17.0.toFigmaHeight(14),
                        color: itemIndex == 5 || itemIndex == 6
                            ? AppColors.colorBlue
                            : null))),
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    final int dayOffset = firstDayOffset(
      year,
      month,
    );
    ...
    while (day < 42 - dayOffset) {
      ...
      if (day < 1) {
        final int pastMonthDaysOffset = firstDayOffset(
          year,
          month - 1,
        );
        dayItems.add(
          _Day(
            ...
            focusNode: _dayFocusNodes[
                pastMonthDaysOffset < 1 ? 0 : pastMonthDaysOffset - 1],
          ),
        );
        ...
        final int nextMonthDaysOffset = firstDayOffset(year, month + 1);
        dayItems.add(
          _Day(
            isOtherMonthDay: true,
            dayToBuild,
            ...
            focusNode: _dayFocusNodes[
                nextMonthDaysOffset < 1 ? 0 : nextMonthDaysOffset - 1],
          ),
        );
      }
    }
...
class _Day extends StatefulWidget {
  const _Day(
    ...
    required this.isOtherMonthDay,
  });

  final DateTime day;
  final bool isDisabled;
  final bool isOtherMonthDay;
  final bool isSelectedDay;
  final bool isToday;
  final ValueChanged<DateTime> onChanged;
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color? dayForegroundColor =
        resolve<Color?>((DatePickerThemeData? theme) {
      if (widget.isToday) {
        return MaterialStatePropertyAll(
            widget.isSelectedDay ? AppColors.colorWhite : AppColors.colorBlue);
      } else {
        return MaterialStatePropertyAll(widget.isSelectedDay
            ? AppColors.colorWhite
            : (widget.day.weekday == 6 || widget.day.weekday == 7)
                ? widget.isOtherMonthDay == false
                    ? AppColors.colorBlue
                    : AppColors.colorSecondaryBlue
                : widget.isOtherMonthDay == true
                    ? AppColors.colorGray
    ...
    final Color? dayBackgroundColor = resolve<Color?>(
        (DatePickerThemeData? theme) => widget.isToday
            ? MaterialStatePropertyAll(widget.isSelectedDay
                ? AppColors.colorBlue
                : AppColors.colorWhite)
            : MaterialStatePropertyAll(widget.isSelectedDay
                ? AppColors.colorBlue
    ...
    final MaterialStateProperty<Color?> dayOverlayColor =
      ...
      (Set<MaterialState> states) => effectiveValue(
          (DatePickerThemeData? theme) => MaterialStatePropertyAll(
                  AppColors.colorSecondaryBlue.withOpacity(0.3))
              .resolve(states)),
    );
      ...
      child: Center(
        child: Text(localizations.formatDecimal(widget.day.day),
            style: AppTextStyle.w400s14
                .copyWith(height: 17.0.toFigmaHeight(14))
...
/// [CustomCalendarDialog], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [CustomCalendarDialog], which provides a Material Design date picker
///    interface.
///
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    final Color? textColor = resolve<Color?>(
        (DatePickerThemeData? theme) => isCurrentYear
            ? MaterialStatePropertyAll(
                isSelected ? AppColors.colorWhite : AppColors.colorBlue)
            : MaterialStatePropertyAll(
                isSelected ? AppColors.colorWhite : AppColors.colorBlack),
        states);
    final Color? background = resolve<Color?>(
        (DatePickerThemeData? theme) => MaterialStatePropertyAll(
            isSelected ? AppColors.colorBlue : AppColors.colorTransparent),
        states);
    ...
    final TextStyle itemStyle = AppTextStyle.w400s16.apply(color: textColor);
